...
| `pontif-receipts` | Receipt-graph subsystem — `Drafter` (deterministic source-to-obligation graph through recursive bodies, match arms, and cross-function calls), `BuiltinIssuer` + `Notary` (default issuer + refutation-only verifier), `IntegerDischarge` layered over `BoundAnalysis` + `SignAnalysis`. Ackermann's `[Int:@>1]` postcondition closes end-to-end. |
