...
- **In-expression `let`** for alt syntax. Today only top-level `let X = value` works (lowers to a 0-arg `IrStmt.FunctionDecl`). Inside expressions, the S-expr parser supports `(let name sort value body)` → `IrExpr.LetIn`, but the alt parser has no equivalent surface. Implementation should be small: reuse `inferMaximalSort`, push `(name, sort)` into `currentScope` for body parsing, emit `IrExpr.LetIn`. Surface form is a design call (`let X = value in body`? `with X = value; body`? — and ties into the "decomposition via `.{...}`" idea where `with` would be the general scope-introducer for value bindings, field decomposition, and qualified-name import).
...
- **Dotted-name lets need parens at use site.** `let Point.origin = Point(0, 0)` declares fine (registered as a dotted dispatch key), but `Point.origin` in expression position parses as `FieldAccess(Var("Point"), "origin")` because the parser's Var→Call rewrite only fires for unqualified single-ident names. Workaround: write `Point.origin()` with parens. Fix: at end of `parsePrimaryWithPostfix`, when the result is a FieldAccess chain whose dotted form matches a `declaredTopLevelLets` key, rewrite to `Call(dotted, [])`.
