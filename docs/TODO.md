...
- **`IrExpr.Record.members` uses `Map.copyOf`, losing insertion order**, inconsistent with `IrSort.Structural` which deliberately wraps a `LinkedHashMap` ("critical for destructure desugaring"). User-written `(record (x 1) (y 2))` may iterate y-first, x-first in evaluation, error messages, and Truffle lowering. Same issue in `Sort.structural(...)` (core/types/Sort.java:17) — even though the IR layer hands it a LinkedHashMap, the canonical constructor's `Map.copyOf` strips the order. Tighten both to unmodifiable `LinkedHashMap`.
...
- **Dead code / stale annotations in the parser frontend:**
  ...
  - `AltToken.EQUALS` (line 28) is documented "reserved for diagnostics; not used by any current form" but `AltParser.parseLet` consumes it at line 476. Update the comment.
...
- **Top-level `let qualified.name:Sort`** with maximally-specific sort. Same synthesis problem spec-only functions handle, but at value level: `let Point.origin:Point[x:0, y:0]` derives the value `(record (x 0) (y 0))`. The qualified name becomes a 0-arg function declared in dispatch.
  - **Footgun today:** `parseLet` (AltParser:476) accepts an `= value` tail and *silently discards* the parsed expression — no warning, no NoOp label hint, the value is gone. Either implement the value-synthesis pipeline or, until then, reject the `= value` form with a clear "not yet supported" error so users don't think their literal `let Point.origin = (record (x 0) (y 0))` is doing anything.
