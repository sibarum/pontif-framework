...
## Item 2 — declaration-keyword registry — DONE (2026-08-25)

**It was FOUR lists, not three,** and the fourth had already drifted: the prose in the dispatch
switch's own error message ("the top level is declarative only (module / requires / … /
conductor)") had been missing `spawn` since that construct landed, so the compiler told users a
keyword they could legally write was not allowed.

**What landed.** One `DECLARATIONS` registry — keyword → parse method — and the three
derivable lists now READ it: `isDeclarationHead` (which `isMainExpressionStart` calls), the
dispatch itself, and both error messages, which are generated by joining its keys. Adding a
construct is one entry. `KEYWORDS` stays separate because it is the LEXER's superset and
legitimately larger (`match`, `emit`, `true`, …); `DeclarationKeywordTest` pins the one
relation that matters — every declaration keyword is a lexical keyword.

**A determinism trap on the way in:** the registry was first written as `Map.ofEntries`, whose
iteration order is salted per JVM run — and that order is USER-VISIBLE, since the error prints
it. The keywords listed themselves in a different order every run. It is a `LinkedHashMap` built
by ordered puts, with a test pinning the order (see the standing "nondeterminism is a trap"
rule).

`module` is deliberately not in the registry: it is the file header, consumed before the
declaration loop. `isDeclarationHead` adds it back for the one question that needs both.
