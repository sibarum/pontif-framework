...
2. **Singleton + empty value braces, with block-expr retirement.** Retire `{EXPR}`; then
   `{x}` = 1-element aggregate, `{}` = empty. Delivers the tree base case `{}` and node-wrap
   `{value}`. (Needs the block-expr ruling — see Open.)
...
- **Block-expr `{EXPR}` retirement** (slice 2 precondition): confirm it goes, and whether
  its closing-boundary role needs a replacement.

## WAR markers (cut sites)

- `AltParser` `LBRACE` case in `parsePrimary` — value aggregate (slice 1 done; slice 2 adds
  `{}`/`{x}` + removes the block fallthrough).
